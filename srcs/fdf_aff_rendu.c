...
void	fdf_aff_horizontale(int i, int j, t_map *m, t_ptr *ptr)
{
	...
	ptr->colors = (m->map[i][j] > m->map[i][j + 1] ? fdf_colo(m->map[i][j], ptr)
		: fdf_colo(m->map[i][j + 1], ptr));
	if (ptr->v1 == 1 && (m->map[i][j] <= ptr->v || m->map[i][j + 1] <= ptr->v))
		return ;
	if ((ptr->x0 < 0 && ptr->x1 < 0) || (ptr->x0 > SIZE_X && ptr->x1 > SIZE_X)
		|| (ptr->y0 < 0 && ptr->y1 < 0)
		|| (ptr->y0 > SIZE_Y && ptr->y1 > SIZE_Y))
		return ;
	mlx_put_line(ptr);
}
...
void	fdf_aff_verticale(int i, int j, t_map *m, t_ptr *ptr)
{
	...
	ptr->colors = 0x00bcbcbc;
	ptr->colors = (m->map[i][j] > m->map[i + 1][j] ? fdf_colo(m->map[i][j], ptr)
		: fdf_colo(m->map[i + 1][j], ptr));
...
void	fdf_aff_diag_1(int i, int j, t_map *m, t_ptr *ptr)
{
	...
	ptr->colors = 0x00bcbcbc;
	ptr->colors = (m->map[i][j] > m->map[i + 1][j + 1]
		? fdf_colo(m->map[i][j], ptr)
		: fdf_colo(m->map[i + 1][j + 1], ptr));
	if (ptr->v1 == 1 && (m->map[i][j] <= ptr->v
		|| m->map[i + 1][j + 1] <= ptr->v))
		return ;
	if ((ptr->x0 < 0 && ptr->x1 < 0) || (ptr->x0 > SIZE_X && ptr->x1 > SIZE_X)
		|| (ptr->y0 < 0 && ptr->y1 < 0)
